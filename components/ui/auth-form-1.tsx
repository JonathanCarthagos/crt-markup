...
import { ArrowLeft, Eye, EyeOff, Loader2, MailCheck, X } from "lucide-react";
...
interface FormState {
  ...
  error: string | null;
  info: string | null;
  errorCode: "existing_email" | "invalid_credentials" | "google_disabled" | "email_rate_limit" | null;
...
function mapAuthError(message: string) {
  ...
  if (normalized.includes("email rate limit exceeded")) {
    return "Too many reset emails sent in a short time. Please wait a few minutes and try again.";
  }
  if (normalized.includes("invalid login credentials")) {
    return "Invalid email or password. If you used magic link before, reset your password first.";
  }
  if (normalized.includes("provider is not enabled")) {
    return "Google login is not enabled yet. Please use email/password for now.";
  }
...
function mapAuthCode(message: string): FormState["errorCode"] {
  const normalized = message.toLowerCase();
  if (normalized.includes("email rate limit exceeded")) return "email_rate_limit";
  if (normalized.includes("invalid login credentials")) return "invalid_credentials";
  if (normalized.includes("provider is not enabled")) return "google_disabled";
  if (normalized.includes("user already registered")) return "existing_email";
  return null;
}
...
function Auth({ className, onClose, onGoogleSignIn, ...props }: React.ComponentProps<"div"> & { onClose?: () => void; onGoogleSignIn?: () => Promise<void>; }) {
  ...
  return (
    <div
      ...
      {...props}
    >
      <div className="relative overflow-hidden rounded-xl border border-border/50 bg-card/100 shadow-xl backdrop-blur-sm">
...
function AuthNotice({ message }: AuthErrorProps) {
  if (!message) return null;
  return (
    <div
      data-slot="auth-notice"
      className="mb-6 animate-in rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-700"
    >
      {message}
    </div>
  );
...
function AuthSocialButtons({ isLoading, onGoogleSignIn }: AuthSocialButtonsProps) {
  return (
    <div data-slot="auth-social-buttons" className="w-full mt-6">
      <Button
        variant="outline"
        className="w-full h-12 bg-background/50 border-border/50 shadow-md"
        disabled={isLoading}
  ...
  return (
    <div data-slot="auth-separator" className="relative mt-6">
      ...
      <div className="flex justify-center text-xs uppercase">
        <span className="px-2 text-black" style={{ backgroundColor: 'rgba(255, 255, 255, 0)' }}>{text}</span>
      </div>
    </div>
  );
}
...
function AuthSignIn({ onForgotPassword, onSignUp, onGoogleSignIn, onClose }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    setFormState((prev) => ({ ...prev, isLoading: true, error: null, info: null, errorCode: null }));
    try {
      ...
      setFormState((prev) => ({ ...prev, error: message, errorCode: mapAuthCode(message) }));
    } finally {
  ...
  return (
    ...
    >
      ...
      <AuthNotice message={formState.info} />
      ...
      <AuthForm onSubmit={handleSubmit(onSubmit)}>
        ...
        <Button type="submit" className="w-full" disabled={formState.isLoading} style={{ backgroundColor: "#FE4004" }}>
          {formState.isLoading ? (
            <>
...
function AuthSignUp({ onSignIn, onForgotPassword, onGoogleSignIn, onClose }: AuthSignUpProps) {
  ...
  const [signupCooldownSeconds, setSignupCooldownSeconds] = React.useState(0);
  ...
  React.useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setSignupCooldownSeconds((prev) => (prev > 0 ? prev - 1 : 0));
    }, 1000);
  ...
  const onSubmit = async (data: SignUpFormValues) => {
    if (signupCooldownSeconds > 0) {
      setFormState((prev) => ({
        ...prev,
        error: `Too many attempts. Please wait ${signupCooldownSeconds}s before trying again.`,
    ...
    try {
      ...
      if (error) {
        const code = mapAuthCode(error.message);
        setFormState((prev) => ({
          ...prev,
          error: mapAuthError(error.message),
          errorCode: code,
  ...
  return (
    ...
    >
      ...
      <AuthForm onSubmit={handleSubmit(onSubmit)}>
        ...
        <Button type="submit" className="w-full" disabled={formState.isLoading || signupCooldownSeconds > 0}>
          {formState.isLoading ? (
            <>
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              Creating account...
      ...
      {formState.errorCode === "existing_email" && (
        <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
          <p className="mb-2">This email already has an account.</p>
          <div className="flex gap-2">
            <Button type="button" variant="outline" className="h-8" onClick={onSignIn}>
              Go to sign in
            </Button>
            <Button type="button" variant="outline" className="h-8" onClick={onForgotPassword}>
              Reset password
            </Button>
          </div>
        </div>
  ...
  const onSubmit = async (data: ForgotPasswordFormValues) => {
    ...
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(data.email, {
        redirectTo: window.location.origin + "/reset-password", // TODO: Create a reset password page
      });
      if (error) {
        ...
        setFormState((prev) => ({
          ...prev,
          info: "Reset link sent. Please check your inbox and spam folder.",
        }));
        onSuccess();
      }
    } catch (err: unknown) {
...
function AuthResetSuccess({ onSignIn, onClose }: AuthResetSuccessProps & { onClose?: () => void }) {
  return (
    <motion.div
